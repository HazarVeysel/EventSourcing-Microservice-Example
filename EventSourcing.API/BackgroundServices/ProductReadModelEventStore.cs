...
namespace EventSourcing.API.BackgroundServices
{
    public class ProductReadModelEventStore : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken) //Uygulama ayağa kalktığında çalışacak olan kısım
        {
        ...
        private async Task EventAppeared(EventStorePersistentSubscriptionBase arg1, ResolvedEvent arg2)
        {
            ...
            using var scope = _serviceProvider.CreateScope(); //kullanılmadığında memory'den düşmesi için scope oluşturduk.
